...
public class Grafica<T> {
    ...
    private class Vertice<T> implements
	    ...
	    Grafica<T>.Vertice<T> n = iterador.next().vecino;
	    n.inicio();
            return n;
	}
	    ...
	    Vertice<T> n=new Vertice<T>(elemento);
	    return n.indice;
	    //Aqui va su código
	}
	...
	while(!mon.esVacio()){
	    Vertice<T> v = mon.elimina();
	    for(Arista<T> a : v.aristas){
		if(a.vecino.distancia == -1 || a.vecino.distancia > v.distancia + a.peso){
		    a.vecino.distancia = v.distancia + a.peso;
		    mon.get(a.vecino.getIndice()).distancia = v.distancia + a.peso;
		    mon.reordena(a.vecino);
		}
	    }
	}
	if(vb.distancia==-1)
