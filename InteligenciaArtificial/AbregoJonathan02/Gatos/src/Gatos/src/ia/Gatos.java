...
public class Gatos extends PApplet {
    ...
    class Gato {
        ...
        LinkedList<Gato> generaSucesores() {
            //verificamos si ya hay un ganador, si lo hay 
            //entonces ya no regresamos nada 
            if (hayGanador) {
                return null;
            }
            //Nuestra lista de sucesores la inicialisamos para conforme vayamos 
            //recorriendo vayamos metiendo los Gatos que se generen
            sucesores = new LinkedList<Gato>();
            //Tenemos uns plar de ciclos for uno dentro de otro para ir haciendo 
            //el recorrido en nuestro tablero de 3x3
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    //verificamos que la entrada i,j no tenga ya un X o O
                    if (tablero[i][j] == 0) { 
                        Gato nuevo = new Gato(this);
                        //Para ir intercalando las tiradas entre X o O
                        if (jugador1 == false) {
                            nuevo.jugador1 = true;
                        } else {
                            nuevo.jugador1 = false;
                        }                        
                        //para ver quien es su padre y no se pierda el hilo
                        nuevo.padre = this;
                       //Como ya verificamos que nuestra entrada (i,j) esta libre 
                       //le indicamos que tire en dicha posicion
                        nuevo.tiraEn(j, i);
                        //out.println(recorreListaGatos(sucesores, nuevo));
                        //Llamamos a nuestar funcion auxiliar para evitar duplicados                        
                        if(recorreListaGatos(sucesores, nuevo))                           
                            //Si es uno nuevo que no teniamos (es decir no es simetrico a otro)
                            // lo agregamos en nuestra lista de sucesores
                            sucesores.add(nuevo);
                    }
                }
            }
            this.sucesores = sucesores;
            //regresamos nuestra lista de sucesores con los gatos 
            return sucesores;
        ...
        //funcion auxiliar que recibe una lista de Gatos y un Gato
        //donde lo que haremos sera que el gato denominado "verificar"
        //lo buscaremos a lo largo de nuestra lista si es igual a alguno ya 
        //existente, donde si existe al menos una igual entonces ya no será 
        //agregado a la lista en el metodo generaSucesores
        //donde lo de las simetrias ya estan implementadas para que funciones 
        //correctamente, dichas simetrias ya fueron hechas como lo vimos en clase         
